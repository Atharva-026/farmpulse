...
router.post('/', async (req, res) => {
  try {
    ...
    if (!vendorId || !listingId || !offeredPrice || !quantityNeeded)
      return res.status(400).json({ success:false, message:'vendorId, listingId, offeredPrice and quantityNeeded are required' });
    ...
    // SSE broadcast to listing watchers (farmer's bids view)
    const clients = global.sseClients?.[listingId.toString()] || [];
...
// PUT /api/bids/:bidId/status — farmer accepts or rejects
router.put('/:bidId/status', async (req, res) => {
  try {
    ...
    // SSE to vendor's personal channel
    const vid = bid.vendorId._id.toString();
    const vendorClients = global.sseVendorBidClients?.[vid] || [];
    vendorClients.forEach(send => send({ type:'BID_STATUS', bidId:bid._id, status, offeredPrice:bid.offeredPrice }));
...
// GET /api/bids/summary/:listingId — pending bid count for notification dot
...
// GET /api/bids/stream/:listingId — SSE real-time updates for BidsView
