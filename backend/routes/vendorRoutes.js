...
router.post('/register', async (req, res) => {
  try {
    ...
    if (existing) {
      return res.status(409).json({ success: false, message: 'Vendor with this phone already exists' });
    }
...
// POST /api/vendor/login  (simple phone-based, no password — extend with OTP later)
router.post('/login', async (req, res) => {
  try {
    ...
    if (!vendor) return res.status(404).json({ success: false, message: 'Vendor not found. Please register first.' });
